...
//        String text = reader.readLine();
        ...
        for (int position = 0; position < text.length(); ++position) {
            ...
            //opening bracket addst s stack
            if (next == '(' || next == '[' || next == '{') {
                // Process opening bracket, write your code here
            	Bracket bracket = new Bracket(next, position );
            	opening_new_brackets.push(bracket);
            	
            	
            	//checks if there is a closing bracket left in string
            	if(!(text.substring(position).contains(")") || text.substring(position).contains("}") || text.substring(position).contains("]"))) {
            		System.out.println( + (position + 1)); //add +1
            		return;
            	}
            }
            	...
            	Bracket top = opening_new_brackets.pop();
            	
            	
            	//checks if tclosing bracket is correct
            	if(!top.Match(next)) {
        ...
        if(opening_new_brackets.empty()) {
        	System.out.println("Success");
        	
        }else {
        	System.out.println( + opening_new_brackets.pop().position);
        }
    }
}
